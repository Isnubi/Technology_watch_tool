<?php
if (($open = fopen($_SERVER['DOCUMENT_ROOT'] . '/csv/cloud_virtualisation_silicon.csv', "r")) !== FALSE) {
    ...
    <div class="card border-primary text-grey text-center p-3 mb-5 rounded shadow-sm">
        ...
        <div class="card-body">
            ...
            <p class="card-text">
                <?php
                echo "Mis à jour le  ";
                if(is_readable($_SERVER['DOCUMENT_ROOT'] . '/csv/cloud_virtualisation_silicon.csv')){
                    $message = date("d/m/Y à H:i:s.", filemtime($_SERVER['DOCUMENT_ROOT'] . '/csv/cloud_virtualisation_silicon.csv'));
                }
                else{
