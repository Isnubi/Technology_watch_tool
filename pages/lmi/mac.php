<?php
if (($open = fopen($_SERVER['DOCUMENT_ROOT'] . '/csv/mac_lmi.csv', "r")) !== FALSE) {
...
<main class="container">
    ...
    <div class="card border-primary text-grey text-center p-3 mb-5 rounded shadow-sm">
        ...
        <div class="card-body">
            <h5 class="card-title">MacOS X</h5>
            <h6 class="card-title">Le monde informatique</h6>
            <p class="card-text">MacOS X, développé par Apple, ...</p>
            <p class="card-text">
                <?php
                echo "Mis à jour le  ";
                if(is_readable($_SERVER['DOCUMENT_ROOT'] . '/csv/mac_lmi.csv')){
                    $message = date("d/m/Y à H:i:s.", filemtime($_SERVER['DOCUMENT_ROOT'] . '/csv/mac_lmi.csv'));
                }
                else{
