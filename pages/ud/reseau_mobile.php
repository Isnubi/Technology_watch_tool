<?php
if (($open = fopen($_SERVER['DOCUMENT_ROOT'] . '/csv/reseau_mobile_ud.csv', "r")) !== FALSE) {
...
<main class="container">
    ...
    <div class="card border-primary text-grey text-center p-3 mb-5 rounded shadow-sm">
        ...
        <div class="card-body">
            <h5 class="card-title">Réseaux mobiles</h5>
            <h6 class="card-title">Usine Digitale</h6>
            <p class="card-text">les réseaux mobiles (4G, 5G, ...) .....</p>
            <p class="card-text">
                <?php
                echo "Mis à jour le  ";
                if(is_readable($_SERVER['DOCUMENT_ROOT'] . '/csv/reseau_mobile_ud.csv')){
                    $message = date("d/m/Y à H:i:s.", filemtime($_SERVER['DOCUMENT_ROOT'] . '/csv/reseau_mobile_ud.csv'));
                }
                else{
